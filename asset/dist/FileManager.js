...
この操作は取り消せません。`))try{const i=e.map(r=>this.deleteFile(r.id.toString())),s=await Promise.allSettled(i),a=s.filter(r=>r.status==="fulfilled").length,n=s.filter(r=>r.status==="rejected").length;a>0&&(h(`${a}件のファイルを削除しました。`),e.forEach(r=>{this.core.removeFile(r.id.toString())})),n>0&&l(`${n}件のファイルの削除に失敗しました。`)}catch(i){console.error("一括削除エラー:",i),l("ファイルの削除中にエラーが発生しました。")}}async moveSelected(e){const t=this.core.getSelectedFiles();if(t.length===0){l("移動するファイルを選択してください。");return}try{const i=t.map(r=>this.moveFile(r.id.toString(),e)),s=await Promise.allSettled(i),a=s.filter(r=>r.status==="fulfilled").length,n=s.filter(r=>r.status==="rejected").length;a>0&&(h(`${a}件のファイルを移動しました。`),t.forEach(r=>{this.core.updateFile(r.id.toString(),{folder_id:e})})),n>0&&l(`${n}件のファイルの移動に失敗しました。`)}catch(i){console.error("一括移動エラー:",i),l("ファイルの移動中にエラーが発生しました。")}}async shareSelected(e){const t=this.core.getSelectedFiles();if(t.length===0){l("共有設定するファイルを選択してください。");return}try{const i=t.map(r=>this.setFileSharing(r.id.toString(),e)),s=await Promise.allSettled(i),a=s.filter(r=>r.status==="fulfilled").length,n=s.filter(r=>r.status==="rejected").length;a>0&&(h(`${a}件のファイルの共有設定を更新しました。`),this.core.refresh()),n>0&&l(`${n}件のファイルの共有設定に失敗しました。`)}catch(i){console.error("一括共有設定エラー:",i),l("共有設定中にエラーが発生しました。")}}async editSelectedMetadata(e){const t=this.core.getSelectedFiles();if(t.length===0){l("編集するファイルを選択してください。");return}try{const i=t.map(r=>this.updateFileMetadata(r.id.toString(),e)),s=await Promise.allSettled(i),a=s.filter(r=>r.status==="fulfilled").length,n=s.filter(r=>r.status==="rejected").length;a>0&&(h(`${a}件のファイル情報を更新しました。`),t.forEach(r=>{this.core.updateFile(r.id.toString(),e)})),n>0&&l(`${n}件のファイル更新に失敗しました。`)}catch(i){console.error("一括メタデータ更新エラー:",i),l("ファイル情報の更新中にエラーが発生しました。")}}getSelectionStats(){const e=this.core.getSelectedFiles(),t={count:e.length,totalSize:0,types:{},oldestDate:null,newestDate:null};return e.forEach(i=>{t.totalSize+=i.size;const s=(i.type||"").split("/")[0];t.types[s]=(t.types[s]||0)+1;const a=new Date(i.upload_date||0);(!t.oldestDate||a<t.oldestDate)&&(t.oldestDate=a),(!t.newestDate||a>t.newestDate)&&(t.newestDate=a)}),t}downloadSingleFile(e){const t=document.createElement("a");t.href=`./download.php?id=${encodeURIComponent(e.id)}`,t.download=e.name||"download",t.style.display="none",document.body.appendChild(t),t.click(),document.body.removeChild(t)}async downloadMultipleFiles(e){try{const t=e.map(s=>s.id),i=await g("/api/files/zip",{file_ids:t});if(i.success&&i.data&&typeof i.data=="object"&&"download_url"in i.data){const s=document.createElement("a");s.href=i.data.download_url,s.download=`files_${new Date().toISOString().slice(0,10)}.zip`,s.style.display="none",document.body.appendChild(s),s.click(),document.body.removeChild(s),h(`${e.length}件のファイルをZIP化してダウンロードしました。`)}else throw new Error(i.error||"ZIP化に失敗しました")}catch(t){console.error("複数ファイルダウンロードエラー:",t),l("複数ファイルのダウンロードに失敗しました。")}}async deleteFile(e){const t=await S(`/api/files/${e}`);if(!t.success)throw new Error(t.error||"ファイルの削除に失敗しました")}async moveFile(e,t){const i=await g(`/api/files/${e}/move`,{folder_id:t});if(!i.success)throw new Error(i.error||"ファイルの移動に失敗しました")}async setFileSharing(e,t){const i=await g(`/api/files/${e}/share`,t);if(!i.success)throw new Error(i.error||"共有設定に失敗しました")}async updateFileMetadata(e,t){const i=await g(`/api/files/${e}/metadata`,t);if(!i.success)throw new Error(i.error||"メタデータの更新に失敗しました")}}class L{core;renderer;events;container;constructor(e,t={}){this.core=new y(e,t),this.renderer=new $(this.core),this.events=new k(this.core),new P(this.core),this.core.setDependencies(this.renderer,this.events),this.container=this.core.container,this.init()}init(){this.core.init()}setFiles(e){this.core.setFiles(e)}getFiles(){return this.core.getFiles()}getFilteredFiles(){return this.core.getFilteredFiles()}getCurrentPage(){return this.core.getCurrentPage()}setPage(e){this.core.setPage(e)}setSearchQuery(e){this.core.setSearchQuery(e)}setSortBy(e,t){this.core.setSortBy(e,t)}setViewMode(e){this.core.setViewMode(e)}getViewMode(){return this.core.getViewMode()}getSelectedFiles(){return this.core.getSelectedFiles()}toggleFileSelection(e){this.core.toggleFileSelection(e)}toggleAllSelection(){this.core.toggleAllSelection()}clearSelection(){this.core.clearSelection()}updateFile(e,t){this.core.updateFile(e,t)}removeFile(e){this.core.removeFile(e)}addFile(e){this.core.addFile(e)}refresh(){this.core.refresh()}getStats(){return this.core.getStats()}getState(){return this.core.getState()}destroy(){this.events.destroy(),this.core.destroy()}loadViewMode(){return this.core.loadViewMode()||"grid"}}export{L as F};
